...
tswlairmgr.modules.inventory.viewFragmenttableRegionRegionalBossFragmentCounts = function inventoryViewFragmenttableRegionRegionalBossFragmentCounts(contentNode, modelInstance, bossInstance, localization) {
	...
	this._build = function() {
		if(tswlairmgr.core.config.debug) console.log("<tswlairmgr.modules.inventory.viewFragmenttableRegionRegionalBossFragmentCounts>: build called");
		$(this._el.self).empty();
	...
	this._build_namesblock = function() {
		...
		this._el.namesBlock.bossNameLine = $("<div />")
			.addClass("bossName");
		this._el.namesBlock.lairNameLine = $("<div />")
			.addClass("lairName");
		
		$(this._el.namesBlock.rootNode).append(this._el.namesBlock.bossNameLine);
		$(this._el.namesBlock.rootNode).append(this._el.namesBlock.lairNameLine);
			...
			for(var col=0; col<sideLength; col++)
			{
				...
				(function(){
					...
					$(".button.plus", cellNode).click(function() {
						self.observables.regionalFragmentCountPlusButtonClicked.notify({
							fragment: fragment
						});
					});
			
					$(".button.minus", cellNode).click(function() {
						self.observables.regionalFragmentCountMinusButtonClicked.notify({
							fragment: fragment
						});
					});
	...
	this._redraw = function() {
		if(tswlairmgr.core.config.debug) console.log("<tswlairmgr.modules.inventory.viewFragmenttableRegionRegionalBossFragmentCounts>: redraw called");
	...
	this._redraw_namesblock = function() {
		...
		$(this._el.namesBlock.lairNameLine).text(this._boss.getLair().getZone().getName() + ": " + this._boss.getLair().getName());
	};
	...
	this._redraw_fragmentsblock = function() {
		...
		$(".fragmentControlsContainer", this._el.fragmentsBlock.rootNode).each(function(index) {
			...
			if(self._model.isLowOnRegionalFragment(fragment))
			{
			...
			if(self._model.getCountForRegionalFragment(fragment) == 0)
			{
			...
			$(".countHave", countsBlock).text(self._model.getCountForRegionalFragment(fragment));
		});
	};
	
	this._redraw_calculatedblock = function() {
		var numberOfFullSets = this._model.getNumberOfSummonsForRegionalBoss(this._boss);
		
		var locString1 = this._localization.getLocalizationData().strings.fragmenttable.calculated.regionalNumberOfFullSets;
		...
		var numberOfMissingForNext = this._model.getNumberOfMissingFragmentsForNextSummonForRegionalBoss(this._boss);
		
		var locString2 = this._localization.getLocalizationData().strings.fragmenttable.calculated.regionalNumberOfMissingForNext;
	...
	this._moduleLocalizationCallback = function(origin, context) {
		if(tswlairmgr.core.config.debug) console.log("<tswlairmgr.modules.inventory.viewFragmenttableRegionRegionalBossFragmentCounts>: got notified that module localization has changed.");
		self._redraw_calculatedblock();
	};
	
	this._dataLocalizationCallback = function(origin, context) {
		if(tswlairmgr.core.config.debug) console.log("<tswlairmgr.modules.inventory.viewFragmenttableRegionRegionalBossFragmentCounts>: got notified that data localization has changed.");
	...
	var self = this;
	this._regionalFragmentCountsCallback = function(origin, context) {
	...
	this._init = function() {
		...
		this._model.observables.regionalFragmentCountsChanged.registerCallback(this._regionalFragmentCountsCallback);
	};
	...
	this.destroy = function() {
		...
		this._model.observables.regionalFragmentCountsChanged.unregisterCallback(this._regionalFragmentCountsCallback);
	};
};