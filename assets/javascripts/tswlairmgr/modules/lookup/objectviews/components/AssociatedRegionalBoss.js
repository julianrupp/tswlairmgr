...
tswlairmgr.modules.lookup.objectviews.components.AssociatedRegionalBoss = function lookupObjectviewComponentAssociatedRegionalBoss(contentNode, regionalBoss, localization) {
	this._regional = regionalBoss;
	...
	this._templates = {
		...
		subTitleLine: '{{context.regionName}}'
	};
	...
	this._build = function() {
		...
		var box = $("<div />")
			.addClass("associatedRegionalBoss");
	...
	this._redraw = function() {
		$(this._el.title).text(
			this._localization.getLocalizationData().strings.objectviewComponents.associatedRegionalBoss.title
		);
		...
		var titleLine = Mustache.render(
			self._templates.titleLine,
			{
				...
				context: {
					bossName: self._regional.getName()
				}
			}
		);
		var subTitleLine = Mustache.render(
			self._templates.subTitleLine,
			{
				localization: self._localization.getLocalizationData(),
				context: {
					regionName: self._regional.getRegion().getName()
				}
			}
		);
		...
		$(rowNode).click(function() {
			tswlairmgr.modules.lookup.controller.setSelectedObject(self._regional);
		});
